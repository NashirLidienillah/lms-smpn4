                        ...
                        <td class="px-6 py-4 text-center">
                            @if($status === 'diblokir' && $hasilUjian)
                                <form id="form-buka-akses-{{ $hasilUjian->id }}" action="/guru/ujian/buka-akses/{{ $hasilUjian->id }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('PATCH')
                                    <button type="button" onclick="konfirmasiBukaAkses('form-buka-akses-{{ $hasilUjian->id }}', '{{ $namaSiswa }}')" class="bg-amber-500 hover:bg-amber-600 text-white px-4 py-2 rounded-xl transition-all text-[10px] font-black flex items-center gap-2 shadow-lg shadow-amber-200 active:scale-95 uppercase tracking-widest mx-auto">
...
<script>
    function konfirmasiBukaAkses(formId, namaSiswa) {
        Swal.fire({
            ...
            icon: 'warning', iconColor: '#f59e0b', showCancelButton: true, confirmButtonColor: '#f59e0b', cancelButtonColor: '#f1f5f9', 
            cancelButtonText: '<span style="color: #475569">Batal</span>', confirmButtonText: 'Ya, Izinkan Kembali!', reverseButtons: true,
            customClass: { confirmButton: 'shadow-lg shadow-amber-200 font-bold text-white', cancelButton: 'font-bold' }
        }).then((result) => {
            if (result.isConfirmed) {
                Swal.fire({ title: 'Membuka Blokir...', text: 'Mohon tunggu sebentar.', allowOutsideClick: false, showConfirmButton: false, didOpen: () => { Swal.showLoading(); }});
