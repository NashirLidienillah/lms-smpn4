...
<div id="konten-tugas" class="grid grid-cols-1 lg:grid-cols-3 gap-8 hidden transition-all">
    <div class="lg:col-span-2 space-y-4">
        @forelse($tugas as $t)
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 relative group overflow-hidden hover:shadow-xl transition-all duration-300">
                <div class="absolute left-0 top-0 h-full w-1.5 bg-blue-500 transition-all group-hover:w-3"></div>
                <div class="flex flex-col md:flex-row justify-between gap-6">
                    <div class="flex gap-5">
                        <div class="w-14 h-14 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center text-2xl shrink-0"><i class="fas fa-file-signature"></i></div>
                        <div>
                            <h4 class="font-bold text-gray-800 text-lg group-hover:text-blue-600 transition-colors">{{ $t->judul }}</h4>
                            ...
                            <a href="/guru/tugas/{{ $t->id }}/koreksi" class="bg-blue-600 hover:bg-blue-700 text-white text-[10px] font-black px-5 py-3 rounded-xl uppercase tracking-widest shadow-lg shadow-blue-100 transition-all flex items-center justify-center w-full sm:w-max">
                                <i class="fas fa-user-edit mr-2"></i> Periksa Lembar Jawaban Siswa
                            </a>
                        </div>
                    </div>
                    <div class="flex sm:flex-col gap-2 shrink-0">
                        <a href="/guru/tugas/{{ $t->id }}/edit" class="w-10 h-10 rounded-xl bg-gray-50 text-gray-400 hover:bg-blue-50 hover:text-blue-600 flex items-center justify-center transition border border-gray-100"><i class="fas fa-pen text-sm"></i></a>
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden sticky top-6">
            ...
            <form action="/guru/kelas/{{ $jadwal->id }}/tugas" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="judul" required placeholder="Contoh: Latihan Soal Bab 1" class="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 transition">
                </div>
                <div>
                    <label class="block text-xs font-black text-gray-500 uppercase mb-2">Instruksi Pembelajaran</label>
                    <textarea name="deskripsi" rows="3" required placeholder="Tuliskan petunjuk pengerjaan tugas di sini..." class="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 transition"></textarea>
                </div>
                <div>
                    <label class="block text-xs font-black text-gray-500 uppercase mb-2">Batas Waktu Pengumpulan</label>
                    <input type="datetime-local" name="batas_waktu" required class="w-full p-3 bg-blue-50 border border-blue-100 rounded-xl text-sm text-blue-700 font-bold focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                ...
                <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-4 rounded-2xl shadow-lg shadow-blue-100 transition-all uppercase tracking-widest text-xs">Simpan Data Tugas</button>
            </form>
        </div>
    </div>
</div>